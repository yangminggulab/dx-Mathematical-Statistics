...
\end{note}
\section*{2.1}
\begin{definition}[总体、样本与抽样]
设 \(X\) 表示研究对象某个指标，其取值分布 \(F_X\)（或总体分布）称为\textbf{总体}。  
通过观察或试验，从总体中获取部分个体指标取值的过程称为\textbf{抽样}。

把一次抽样结果看成随机向量
\[
(X_1,X_2,\cdots,X_n)^{\mathrm T},
\]
则称其为来自总体 \(X\)（或 \(F_X\)）的一个\textbf{样本}；其概率分布称为\textbf{样本分布}。  
对应的具体取值
\[
(x_1,x_2,\cdots,x_n)
\]
称为\textbf{样本观察值}。其中 \(n\) 称为\textbf{样本大小}，也称\textbf{样本容量}。
\end{definition}
\begin{dxtips}[概率论里面学的都是总体]
  
\begin{enumerate}
    \item 概率论里，通常先给定随机变量 \(X\) 的分布，再研究它的性质；
    \item 数理统计里，则把研究对象的某个指标记为 \(X\)，把它的分布看作总体分布，再根据样本去反推这个分布中的未知信息。
\end{enumerate}

\end{dxtips}
\begin{definition}[简单随机样本]
若样本 \(X_1,X_2,\cdots,X_n\) 满足：
\begin{itemize}
    \item 各分量同分布，即每个 \(X_i\) 都与总体 \(X\) 有相同分布；
    \item 各分量相互独立，
\end{itemize}
则称 \(X_1,X_2,\cdots,X_n\) 为来自总体 \(X\) 的\textbf{简单随机样本}，记作
\[
X_1,X_2,\cdots,X_n \text{ i.i.d.}
\]
若总体分布函数为 \(F(x)\)（或概率密度为 \(f(x)\)），常记作
\[
X_1\sim F(x)\qquad (\text{或 }X_1\sim f(x)).
\]
\end{definition}
\begin{dxtips}
    从X中选取具体的X1就是抽样，一次抽样并不意味着只得到一个数；一次抽样可以抽取 \(n\) 个个体，从而得到一个容量为 \(n\) 的样本
\[
X_1,X_2,\dots,X_n.
\]
...
\end{dxtips}
\begin{example}
设总体 \(X\sim E(\lambda)\)，其概率密度为
\[
f(x;\lambda)=
\begin{cases}
\dfrac{1}{\lambda}e^{-x/\lambda}, & x>0,\\[0.5em]
0, & \text{其他}.
\end{cases}
\]
若 \(X_1,X_2,\cdots,X_n\) 为来自该总体的简单随机样本，则样本分布为
\[
\prod_{i=1}^n f(x_i;\lambda)
=
\begin{cases}
\dfrac{1}{\lambda^n}\exp\!\left(-\dfrac{1}{\lambda}\sum_{i=1}^n x_i\right), & x_1,x_2,\cdots,x_n>0,\\[0.8em]
0, & \text{其他}.
\end{cases}
\]
\end{example}
...
\begin{note}
所以这个概率分布就是一次抽样抽到这个样本的概率对吗也就是100个观测值概率的乘积；若总体是连续型，这里写出的则是联合概率密度，而不是单点概率。
\end{note}
\begin{definition}[参数、参数空间与分布族]
总体分布中出现的常数称为\textbf{参数}；若有多个参数，它们组成的向量称为\textbf{参数向量}；其值未知时称为\textbf{未知参数}。  
参数所有可能取值构成的集合称为\textbf{参数空间}，常记为 \(\Theta\)。
...
\end{definition}
\begin{note}
在数理统计中，未知的通常不是样本本身，而是总体分布中的参数。比如正态总体 \(N(\mu,\sigma^2)\) 中未知的是 \(\mu,\sigma^2\)，指数分布 \(E(\lambda)\) 中未知的是 \(\lambda\)，二项分布 \(B(n,p)\) 中未知的是 \(p\)。因此把总体写成
\[
f(x;\theta)
\]
就是在说明：分布的形式已知，但其中参数 \(\theta\) 的真实取值未知。这样一来，问题就从“研究一个模糊的总体”转化为“研究未知参数 \(\theta\)”。
\end{note}
\begin{definition}[统计模型与参数统计问题]
样本分布族称为\textbf{统计模型}。  
若总体分布形式已知，只需对其中未知参数作出推断，则称这类问题为\textbf{参数统计问题}。
\end{definition}

\begin{note}
因此，统计模型通常由两部分确定：一是参数空间 \(\Theta\)，二是由 \(\Theta\) 所决定的样本分布族。换言之，所谓“建立统计模型”，本质上就是说明样本来自哪一族分布，以及其中哪些参数是未知的。
\end{note}
\begin{definition}[统计量与抽样分布]
设 \(X_1,X_2,\cdots,X_n\) 是来自总体的样本。若一个量
\[
T=T(X_1,X_2,\cdots,X_n)
\]
是样本的完全已知函数，即它只依赖于样本而不含未知参数，则称 \(T\) 为\textbf{统计量}。
...
由于样本 \((X_1,X_2,\cdots,X_n)\) 是随机向量，统计量作为样本的函数也是随机变量；统计量的概率分布称为它的\textbf{抽样分布}。
\end{definition}
...
数学家们发现，这个离散的求和公式，竟然可以完美地写成一个连续积分的形式：作业2.2.3
...
\end{definition}
\begin{dxtips}
    看到inf也就是下极限，还有sup也就是知道他的出现只是为了解决离散的点或者极限不在集合里面比如 $x \in (0,1)$ 他没有最大值但是有上界
\end{dxtips}
...
\end{definition}
\begin{dxtips}[卡方分布要记的公式]
    $E(Y)=k,\qquad \operatorname{Var}(Y)=2k.$
\end{dxtips}
\begin{note}
自由度更准确地说，是指真正能自由变化的独立信息个数。

例如，若
\[
Y=Z_1^2+\cdots+Z_n^2,\qquad Z_1,\dots,Z_n \stackrel{\text{i.i.d.}}{\sim} N(0,1),
\]
