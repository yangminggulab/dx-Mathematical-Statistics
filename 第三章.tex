...
\end{note}
\begin{theorem}[MLE 相合性（Consistency）定理]
设分布族 $\{f(x;\theta):\theta\in\Theta\}$ 满足：
\begin{enumerate}[(1)]
    \item $\Theta$ 为 $\mathbf{R}$（一维实空间）中的开集；\textcolor{red}{开集保证可以对$\theta$求导}
    \item 不同的参数值 $\theta$ 和 $\theta'$ 所对应的分布不同；
    \textcolor{red}{如果不同θ分布相同就分不清那个是真实参数}
    \item $\{f(x;\theta):\theta\in\Theta\}$有共同支撑 $A$；\textcolor{red}{保证似然函数的结构在整个参数空间上一致，也就是似然方程有意义}
    \item $f(x;\theta)$ 对 $\theta$ 的偏导数 $\dfrac{\partial f(x;\theta)}{\partial\theta}$ 在 $\Theta$ 上存在，并且当简单随机样本 $(X_1,X_2,\cdots,X_n)^\mathrm{T}\in A^n$ 时，似然方程
    $$
    \frac{\partial}{\partial\theta}\prod_{i=1}^n f(X_i;\theta) = 0
    $$
    有唯一解 $\hat{\theta}_n = \hat{\theta}(X_1,X_2,\cdots,X_n)$，\textcolor{red}{保证 MLE 存在且唯一——有唯一解才能保证估计量是良定义的，才能谈收敛到真实值。}
\end{enumerate}
则 $\hat{\theta}_n \xrightarrow{P} \theta\ (n\to\infty)$，即 $\hat{\theta}_n$ 为 $\theta$ 的相合估计量。
\end{theorem}
\begin{note}
本定理说的是：在一定条件下，MLE 是相合估计量——即样本量 $n\to\infty$ 时，最大似然估计量依概率收敛到真实参数值。
\textbf{相合性（Consistency）}：估计量随样本量增大越来越准，极限上依概率收敛到真实参数值，即
$$
\hat{\theta}_n \xrightarrow{P} \theta \quad (n \to \infty).
$$
直觉就是：数据越多，估计越靠谱，样本无穷大时误差趋于0。
一句话总结：(1)(3)(4) 保证 MLE 能算出来且唯一，(2) 保证算出来的东西有意义。
\end{note}
\begin{definition}[渐近正态性]
若 $g(\theta)$ 的估计 $\hat{g}_n = \hat{g}_n(X_1,X_2,\cdots,X_n)$ 满足
$$
\sqrt{n}(\hat{g}_n - g(\theta)) \xrightarrow{L} N(0,V(\theta)) \quad (n\to\infty),
$$
则称估计量 $\hat{g}_n$ 是\textbf{渐近正态的}。由上式，当 $n$ 充分大时，近似地有
$$
\hat{g}_n \sim N\!\left(g(\theta),\, \frac{V(\theta)}{n}\right),
$$
称该分布为估计量 $\hat{g}_n$ 的\textbf{渐近分布}，$V(\theta)/n$ 称为 $\hat{g}_n$ 的\textbf{渐近方差}。
\end{definition}
\begin{note}
本定义说的是：估计量 $\hat{g}_n$ 的误差经过 $\sqrt{n}$ 放大后，在大样本下近似服从正态分布。

直觉是：单个估计误差 $\hat{g}_n - g(\theta)$ 随 $n$ 增大趋于 $0$，太小了看不清楚形状，乘以 $\sqrt{n}$ 把它"放大"到合适的尺度，这时候形状依分布收敛（$\xrightarrow{L}$，即弱收敛）到正态分布。

实际用途是：样本量足够大时，可以直接用 $\hat{g}_n \sim N(g(\theta), V(\theta)/n)$ 来做区间估计和假设检验，不需要知道 $\hat{g}_n$ 的精确分布。
\end{note}
L是来自法语 "Loi"，意思是"分布/定律"，法国概率学派的传统记号。
\begin{theorem}[ MLE 渐近正态性]
设 $X_1,X_2,\cdots,X_n$ i.i.d.，$X_1\sim f(x,\theta)$，$\theta\in\Theta$，分布族 $\{f(x;\theta):\theta\in\Theta\}$ 满足：
\begin{enumerate}[(1)]
    \item $\Theta$ 是开区间；\textcolor{red}{（保证可以对 $\theta$ 求导）}
    \item $A=\{x:f(x;\theta)>0\}$ 与 $\theta$ 无关，即分布族有共同支撑；\textcolor{red}{（保证似然函数结构一致，对所有 $\theta$ 有意义）}
    \item $\forall x\in A$，$f(x;\theta)$ 对 $\theta$ 三次可微，且三阶导数是 $\theta$ 的连续函数；\textcolor{red}{（保证可以对似然函数做 Taylor 展开）}
    \item 对积分 $\displaystyle\int_A f(x;\theta)\mathrm{d}x$ 可在积分号下对 $\theta$ 微分两次；\textcolor{red}{（保证期望和导数可以交换顺序，Fisher 信息量的两个等价公式成立）}
    \item Fisher 信息量 $I(\theta)$ 满足 $0<I(\theta)<\infty$；\textcolor{red}{（保证渐近方差 $1/I(\theta)$ 有意义且非退化）}
    \item $\forall\theta_0\in\Theta$，存在正数 $a$ 和函数 $M(x)$，使得
    $$
    \left|\frac{\partial^3\ln f(x;\theta)}{\partial\theta^3}\right| \leq M(x), \quad x\in A,\ \theta_0-a<\theta<\theta_0+a
    $$
    及 $E_{\theta_0}(|M(X)|)<\infty$；\textcolor{red}{（控制 Taylor 展开余项，保证展开合法）}
\end{enumerate}
设 $\hat{\theta}_n=\hat{\theta}_n(X_1,X_2,\cdots,X_n)$ 为似然方程的根，并且满足 $\hat{\theta}_n\xrightarrow{P}\theta_0$，则
$$
...
\begin{theorem}[Delta 方法--也就是原来相当于y=x现在更普遍]
设 $\hat{\theta}_n$ 为 $\theta$ 的估计量，满足
$$
\sqrt{n}(\hat{\theta}_n - \theta) \xrightarrow{L} N(0, V(\theta)),
$$
$h(\theta)$ 在 $\Theta$ 上可导，且 $h'(\theta)\neq 0$，则
$$
\sqrt{n}(h(\hat{\theta}_n) - h(\theta)) \xrightarrow{L} N(0,(h'(\theta))^2 V(\theta)) \quad (n\to\infty).
$$
\end{theorem}
那导乘外导罢了
\begin{corollary}[在正则条件下，MLE 的渐近方差恰好达到 C-R 下界]
在定理 3.4.5 和定理 3.4.6 的条件下，有
$$
\sqrt{n}(h(\hat{\theta}) - h(\theta)) \xrightarrow{L} N\!\left(0, \frac{[h'(\theta)]^2}{I(\theta)}\right).
$$
注意，上式表明 $h(\theta)$ 的最大似然估计量 $h(\hat{\theta})$ 的渐近方差与 C-R 下界一致。这就是说，在该推论的正则条件下，最大似然估计量 $h(\hat{\theta})$ 的渐近方差达到了 C-R 下界，称这样的估计量为\textbf{有效估计量}。
\end{corollary}
\begin{note}[模拟估计方法比较]
三种方法的核心区别在于对总体分布 $F$ 的已知程度：
\begin{itemize}
    \item \textbf{简单模拟法：} $F$ 完全已知，直接从 $F$ 中抽取 $k$ 组样本，用样本方差估计 $\mathrm{Var}(T)$。优点：最简单直接；缺点：要求分布完全已知，实际中很少满足。

    \item \textbf{参数自助法：} $F_c$ 类型已知但参数 $c$ 未知，先用观测数据估计 $\hat{c}$，再从 $F_{\hat{c}}$ 中产生 $B$ 组样本，计算
    $$
    \widehat{\mathrm{MSE}}_{pb}(T) = \frac{1}{B}\sum_{j=1}^B (T^{(j)} - \theta(F_{\hat{c}}))^2.
    $$
    优点：利用了分布类型的先验信息，估计更精准；缺点：依赖分布类型假设正确。

    \item \textbf{一般自助法（非参数自助法）：} $F$ 完全未知，用经验分布函数 $F_n$ 代替 $F_{\hat{c}}$，即在 $\{x_1,x_2,\cdots,x_n\}$ 中\textbf{有放回地}抽取容量为 $n$ 的样本 $B$ 次，计算每组样本下 $T^{(j)}$，再估计方差。优点：无需任何分布假设，适用范围最广；缺点：完全依赖样本，小样本下效果差。
\end{itemize}
\end{note}
\section{渐进相对效率}
\begin{note}
    设 $(X_1, X_2, \cdots, X_n)^\mathrm{T}$ 为来自于分布 $F(x,\theta)$ 的样本，$\hat{\theta}_{1n}$ 和 $\hat{\theta}_{2n}$ 为 $\theta$ 的两个不同的估计量，分别满足
\[
\sqrt{n}(\hat{\theta}_{1n} - \theta) \xrightarrow{L} N(0, \sigma_1^2), \qquad \sqrt{n}(\hat{\theta}_{2n} - \theta) \xrightarrow{L} N(0, \sigma_2^2),
\]
称 $\sigma_1^2/\sigma_2^2$ 为 $\hat{\theta}_{2n}$ 相对于 $\hat{\theta}_{1n}$ 的\textbf{渐近效率}．
\end{note}
\begin{theorem}[渐近效率]


进一步，设 $\hat{\theta}_n$ 为 $\theta$ 的基于样本 $(X_1, X_2, \cdots, X_n)^\mathrm{T}$ 的估计量，满足 $\sqrt{n}(\hat{\theta}_n - \theta) \xrightarrow{L} N(0, \sigma_1^2)$．若样本容量 $n$ 的整函数 $m = m(n)$ 满足
\[
\lim_{n \to \infty} \frac{m(n)}{n} = \gamma \quad (\gamma > 0),
\]
则
\[
\sqrt{n}\!\left(\hat{\theta}_{m(n)} - \theta\right) \xrightarrow{L} N\!\left(0,\ \frac{\sigma_1^2}{\gamma}\right).
\]
\end{theorem}
\begin{dxtips}
    渐近效率本质上是在回答一个问题：两个估计量，谁用更少的数据就能达到同样的精度？
  
    \textbf{核心直觉}

$\hat{\theta}_{2n}$ 相对于 $\hat{\theta}_{1n}$ 的渐近效率是 $e = \sigma_1^2/\sigma_2^2$．这个比值的含义是：用 $\hat{\theta}_{1n}$ 需要 $n$ 个样本才能达到的精度，用 $\hat{\theta}_{2n}$ 只需要 $en$ 个样本就够了．定理 3.4.7 正是在严格化这句话——如果把样本量从 $n$ 缩减到 $m(n) \approx \gamma n$，估计量的渐近方差就从 $\sigma_1^2$ 膨胀到 $\sigma_1^2/\gamma$．

\end{dxtips}
\textbf{典型用途}

\begin{enumerate}
    \item \textbf{比较两个估计量的优劣．}例如同样估计正态分布的均值 $\mu$：样本均值 $\overline{X}$ 的渐近方差是 $\sigma^2$，样本中位数的渐近方差是 $\pi\sigma^2/2$，故样本中位数相对于 $\overline{X}$ 的渐近效率为 $2/\pi \approx 0.637$，意味着中位数需要多用约 $57\%$ 的样本才能达到均值的精度．

    \item \textbf{衡量估计量离最优有多远．}将 $\hat{\theta}_{1n}$ 取为渐近方差恰好达到 C-R 下界 $1/I(\theta)$ 的估计量（如 MLE），则
    \[
        e = \frac{1/I(\theta)}{\sigma_2^2}.
    \]
    若 $e = 1$，称 $\hat{\theta}_{2n}$ 是\textbf{渐近有效的}，即大样本下已无法做得更好．

    \item \textbf{在有限资源下做决策．}若收集数据有成本，渐近效率直接告诉我们：换一个效率为 $e$ 的估计量，等价于把样本量乘以 $e$．效率为 $0.8$ 的估计量意味着多花了 $25\%$ 的采样成本．
\end{enumerate}
\subsection{区间估计}
\begin{definition}[置信区间]
设 $X_1, X_2, \cdots, X_n$ i.i.d.，$X_1 \sim f(x;\theta)$，$\theta \in \Theta$，$0 < \alpha < 1$ 为一个给定的数．若统计量 $\theta_1(\boldsymbol{X}) = \theta_1(X_1, X_2, \cdots, X_n)$，$\theta_2(\boldsymbol{X}) = \theta_2(X_1, X_2, \cdots, X_n)$ 满足 $\theta_1(\boldsymbol{X}) \leqslant \theta_2(\boldsymbol{X})$，并且
\[
P_\theta\{\theta_1(\boldsymbol{X}) \leqslant \theta \leqslant \theta_2(\boldsymbol{X})\} \geqslant 1 - \alpha, \quad \forall \theta \in \Theta,
\]
则称 $[\theta_1(\boldsymbol{X}), \theta_2(\boldsymbol{X})]$ 是参数 $\theta$ 的置信度为 $100(1-\alpha)\%$ 的\textbf{置信区间}．若
\[
P_\theta\{\theta \leqslant \theta_2(\boldsymbol{X})\} \geqslant 1 - \alpha, \quad \forall \theta \in \Theta,
\]
则称 $\theta_2(\boldsymbol{X})$ 是参数 $\theta$ 的置信度为 $100(1-\alpha)\%$ 的\textbf{置信上限}．若
\[
P_\theta\{\theta \geqslant \theta_1(\boldsymbol{X})\} \geqslant 1 - \alpha, \quad \forall \theta \in \Theta,
\]
则称 $\theta_1(\boldsymbol{X})$ 是参数 $\theta$ 的置信度为 $100(1-\alpha)\%$ 的\textbf{置信下限}．
\end{definition}
\begin{dxtips}
    置信区间回答的问题是：能否用数据构造一个区间，使其大概率覆盖未知参数 $\theta$？约可靠需要的区间长度越长，区间长度越长精度就越低
\end{dxtips}
\begin{note}


$\theta$ 是固定的未知常数，而 $[\theta_1(\boldsymbol{X}), \theta_2(\boldsymbol{X})]$ 是随机的，随样本变化．置信度 $1-\alpha$ 的正确含义是：在相同条件下重复抽样，每次用同样的方法构造区间，其中至少有 $100(1-\alpha)\%$ 的区间会包含真实的 $\theta$．

注意一个常见误解：置信度 $1-\alpha$ \textbf{并非}指"$\theta$ 有 $1-\alpha$ 的概率落在该区间内"——$\theta$ 是常数而非随机变量，随机的是区间本身，而非 $\theta$．

置信上限与置信下限对应单侧约束：置信上限关心 $\theta$ 不超过某值，置信下限关心 $\theta$ 不低于某值，双侧约束即为通常的置信区间．
\end{note}
\begin{theorem}[枢轴量方法]
构造置信区间的枢轴量方法概括如下：
\begin{enumerate}
    \item 找一个与被估计量 $\theta$ 有关的统计量 $T = T(\boldsymbol{X})$，通常可选取一个点估计量；
    \item 构造 $T$ 和被估计量 $\theta$ 的一个函数 $G(T, \theta)$，使得其分布与未知参数无关，$G(T, \theta)$ 称为\textbf{枢轴量}；
    \item 选取 $G(T, \theta)$ 的一个取值范围 $S$，满足 $P(G(T,\theta) \in S) = 1 - \alpha$，此时 $S$ 与未知参数无关；
    \item 将 $G(T, \theta) \in S$ 作等价变换，化为 $\theta_1(\boldsymbol{X}) \leqslant \theta \leqslant \theta_2(\boldsymbol{X})$ 的形式，则 $[\theta_1(\boldsymbol{X}), \theta_2(\boldsymbol{X})]$ 即是置信度为 $100(1-\alpha)\%$ 的置信区间．
\end{enumerate}
\end{theorem}
\begin{note}
``枢轴''的意思是转动的轴心，$G(T,\theta)$ 充当的角色正是如此：它一边连着数据（通过 $\overline{X}$），一边连着未知参数（通过 $\mu$），但它自己的分布是固定已知的．可以把它想象成一个翻译器：左边输入数据，右边输出关于 $\mu$ 的约束，中间的概率计算完全不需要知道 $\mu$．
\end{note}
\begin{dxtips}
        也就这个流程就是先找到一个T，T必须和θ相关，也就是正态分布的平均数就是θ，指数分布的拉姆达，和均匀分布的θ，然后T得是渐进正态或者正态的，然后把T标准化根据置信区间得到标准化完了以后的范围然后解方程对吗
       
正态、指数、均匀等分布的充分统计量 $T$ 的精确分布已知，无需渐近，可直接精确变换为 $N(0,1)$、$\chi^2$ 或 $\text{Beta}$ 等已知分布；对于其他分布，精确分布难以求得，此时借助 CLT 说明 $T$ 渐近正态，再标准化得到渐近枢轴量．

\end{dxtips}
\subsection{正态的性质们}
\begin{note}
设 $Z \sim \mathcal{N}(0,1)$，令 $Q = Z^2$，则 $Q \sim \chi^2(1)$。

\textbf{变换方向}：$q = z^2$，绝对值越大 $q$ 越大：
$$|z| > 1 \Rightarrow q > 1 \text{（往右移）}, \quad |z| < 1 \Rightarrow q < 1 \text{（往左移）}$$

\textbf{密度变换}（换到 $Q$ 的空间，需要雅可比修正）：

$\pm z$ 映射到同一个 $q$，两侧叠加，区间从 $2\,dz$ 压缩到 $dq$：
$$f_Q(q) = 2 \cdot f_Z(\sqrt{q}) \cdot \frac{1}{2\sqrt{q}} = \frac{f_Z(\sqrt{q})}{\sqrt{q}}, \quad q > 0$$

\textbf{求期望}（在 $Z$ 的空间积分，密度不变）：
$$E[Q] = E[Z^2] = \int_{-\infty}^{+\infty} z^2 f_Z(z)\, dz = 1$$

\textbf{关键区别}：求期望是在原空间加权平均，积分变量未换，密度不动；变量变换是换了积分变量，必须乘雅可比项保证概率守恒。
\end{note}