...
对 \(p\) 求导，得
\[
S'(p)=\sum_{k=r}^{n} C_n^k \Bigl[kp^{k-1}(1-p)^{n-k}-(n-k)p^k(1-p)^{n-k-1}\Bigr].
\]
利用组合恒等式
\[
...
于是两边从 \(0\) 到 \(p\) 积分，
\[
S(p)-S(0)=rC_n^r\int_0^p t^{r-1}(1-t)^{n-r}\,dt.
\]
当 \(r\ge 1\) 时，
\[
S(0)=\sum_{k=r}^{n} C_n^k 0^k(1-0)^{n-k}=0.
\]
故
\[
